...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match='/'>
    <rss version="2.0">
      <channel>
        <title><xsl:value-of select="$channelDomain"/> - jobsintown.de GmbH - RSS 2.0 Feed</title>
        <link>http://<xsl:value-of select="$channelDomain"/>/</link>
        <description>RSS feed for <xsl:value-of select="$channelDomain"/> job announcements.</description>
  ...
  <xsl:template match="doc">
    ...
    <item>
        ...
        <link>http://<xsl:value-of select="$channelDomain"/>/Stellenangebote.<xsl:value-of select="$id"/>.<xsl:value-of select="$channelPageId"/>.0.html</link>
        <guid>http://<xsl:value-of select="$channelDomain"/>/Stellenangebote.<xsl:value-of select="$id"/>.<xsl:value-of select="$channelPageId"/>.0.html</guid>
